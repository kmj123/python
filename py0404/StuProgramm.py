    ...
    if choice == 1: # 학생성적입력부분
        ...
        for s in students:
            if s['name'] == name:
                ...
                if choice == 1:
                    ...
                    s['avg'] = s['total']/3 
                    print(f"변경전 : {pre_score}점을 {s[sub_list[choice]]}점으로 수정되었습니다.")
                elif choice == 2:
                    pre_score = s[sub_list[choice]]
                    ...
                    pre_score = s['math']
                    print(f"현재 {title[choice+1]}점수 : {pre_score}")
                    s['math'] = int(input(f"수정할 {title[choice+1]}점수 입력 : "))
                    s['total'] = s['kor']+s['eng']+s['math']
                    s['avg'] = s['total']/3
                    print(f"변경전 : {pre_score}점을 {s['math']}점으로 수정되었습니다.")
        ...
        for i,s in enumerate(students):
            if name == s['name']:
                ...
                answer = input("학생성적을 삭제할까요?(*삭제하시면 복구불가)")
