# 클래스 형태 - 파일구성: 다른타입형태 넣을 수 있다는 장점

# 변수 -> 배열(같은 타입) -> 다른타입(구조체) -> 객체(클래스) - 변수, 함수
# 객체 지향언어 - 고급언어
...
# class 첫글자 대문자 사용, 강제성은 아니지만 대부분 대문자 사용/ (권장사항)
# 설계도
class Stu:
...
# 직접저장이 아닌 간접 저장을 하는 이유
# 유지보수가 쉽고 데이터 접근을 막을수있다.      
